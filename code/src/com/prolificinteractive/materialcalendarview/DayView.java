...
public class DayView extends CheckedTextView {
	...
	public DayView(Context context, AttributeSet attrs) {
		...
		fadeTime = getResources().getInteger(
	...
	public void setDay(boolean flag, CalendarDay date) {
		this.date = date;
		if (flag) {
			int num = date.getWeekOfYear();
			num = num - 9;
			if (num < 1 || num > 19)
				setText("");
			else {
				setText(num + "");
				setTextColor(Color.parseColor("#0b9ede"));
				setTextSize(16);
				setEnabled(false);
			}
	...
	protected void setupSelection(boolean showOtherDates, boolean inRange,
			boolean inMonth) {
	...
	public static Drawable generateCircleDrawable(final int color) {
		...
		drawable.setShaderFactory(new ShapeDrawable.ShaderFactory() {
			...
			public Shader resize(int width, int height) {
				return new LinearGradient(0, 0, 0, 0, color, color,
	...
	public static Drawable generateRippleDrawable(final int color) {
