...
import cn.edu.university.zfcms.ui.course.activity.CourseActivity;
import cn.edu.university.zfcms.ui.course.adapter.DayCoursesAdapter;
import cn.edu.university.zfcms.model.entity.Course;

/**
 * Created by hjw on 16/4/15.
 */
public class DayCoursesFragment extends BaseFragment implements CourseContract.View {
    ...
    protected void initNoLazyData() {
        adapter = new DayCoursesAdapter(App.getInstance(), getFragmentManager());
        pager.setAdapter(adapter);
        pager.setOffscreenPageLimit(adapter.getCount());
        ((CourseActivity) getActivity()).getTabs().setupWithViewPager(pager);
    }
